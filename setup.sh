...
if [ "$(id -u)" == "0" ]; then
	...
	adir=$(pwd)
	cd /
	echo -e "\n${blueColour}[${endColour}${yellowColour}SIDRA${endColour}${blueColour}] Preparing the setup for working properly.${endColour}"
	...
	if [ ! -f "/opt/wef/main/wordlists/rockyou.txt" ]; then
		log_progress "Downloading necesary files" &
		l=$!
	...
	echo -e "\n${blueColour}[${endColour}${yellowColour}X${endColour}${blueColour}] Execute the script as root\n${endColour}"
	exit 1
fi
